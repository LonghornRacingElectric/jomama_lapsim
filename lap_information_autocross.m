...
global path_boundaries_ax r_min r_max cornering accel deccel lateral...
    top_speed 
...
for i = 1:1:length(segment)
    ...
    AX = accel;%fnval(accel,vel);
    AY = lateral;%fnval(lateral,vel);
    dd = d/interval;
    for j = 1:1:interval
        count = count+1;
...
AY_outlier = find(lateral_accel > 1.1);%fnval(lateral,116));
lateral_accel(AY_outlier) = 1.1;%fnval(lateral,116);
throttle = 0;
brake = 0;
corner = 0;
for i = 1:1:length(VD)
